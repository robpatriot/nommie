...
use support::game_phases::{setup_game_in_bidding_phase, setup_game_in_trick_play_phase};
use support::game_setup::setup_game_with_players;
use support::trick_helpers::{create_tricks_by_winner_counts, create_tricks_with_winners};
use tracing::info;
...
async fn test_game_completes_after_final_round() -> Result<(), AppError> {
    ...
    with_txn(None, &state, |txn| {
        Box::pin(async move {
            let setup = support::game_phases::setup_game_at_round(txn, 12345, 25).await?;
            let service = GameFlowService::new();
