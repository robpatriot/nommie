...
// 2. Attempting to update with a stale version → OptimisticLock conflict
//
...
async fn test_update_nonexistent_game_returns_not_found() -> Result<(), AppError> {
    ...
    let result = with_txn(None, &state, |txn| {
        Box::pin(async move {
            ...
            let update_dto =
                GameUpdate::new(non_existent_id, expected_version).with_state(GameState::Bidding);
...
/// Test that attempting to update with a stale version returns an OptimisticLock conflict
...
async fn test_optimistic_lock_conflict_returns_expected_and_actual() -> Result<(), AppError> {
    ...
    let result = with_txn(None, &state, |txn| {
        Box::pin(async move {
            // 1. Create a game (initial version should be 1)
            ...
            // 2. First update: succeed with correct version
            ...
            // 3. Second update: attempt with stale version (use the original version 1)
            let stale_version = initial_version; // Still 1, but actual is now 2
            ...
            match domain_err {
                DomainError::Conflict(kind, detail) => {
                    ...
                    assert!(
                        detail.contains(&format!("actual version {}", initial_version + 1)),
...
async fn test_multiple_stale_updates_all_fail() -> Result<(), AppError> {
    ...
    let result = with_txn(None, &state, |txn| {
        Box::pin(async move {
            ...
            assert_eq!(game_v2.version, initial_version + 1);

            // Successfully update again
            let update2 =
                GameUpdate::new(game.id, game_v2.version).with_state(GameState::TrickPlay);
            let game_v3 = games_sea::update_game(txn, update2)
                .await
                .map_err(|e| AppError::db("failed to update game state", e))?;

            assert_eq!(game_v3.version, initial_version + 2);
