 ...
 * - 401 UNAUTHORIZED: No session cookie available
 * - 401 FORBIDDEN_USER_NOT_FOUND: Session valid but user no longer in DB (e.g. DB wipe)
 */
export function isStaleSessionError(error: unknown): boolean {
  ...
  const staleCodes = ['FORBIDDEN_USER_NOT_FOUND', 'UNAUTHORIZED']
